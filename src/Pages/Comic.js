...
import { STATUS } from '../Store/apiSlice2'


import { fetchapi } from '../Store/apiSlice2'
...
export default function Comic() {
    ...
    //here data and status is fetch by useSelector can be used for rendering all the search Comics data
    const data = useSelector((state) => state.api2.data)
    const status = useSelector((state) => state.api2.status)
                ...
                <div className='flex flex-col mx-[auto] w-[300px] h-[100px] lg:h-[492px]  lg:border-r-[1px]  lg:border-[#adadad]  '>
                    ...
                    <div className='flex flex-row w-full mx-[auto] lg:flex-col'>
                        ...
                        <div class="flex items-center mb-4 mr-[auto] sm:mx-[auto]">
                            <input type="radio" name="orderby" value="focDate" onClick={Input}></input>
                            <label for="default-radio-1" class="ml-3 text-sm font-medium text-gray-900 dark:text-gray-300">Foc Date</label>
                        </div>
                        <div class="flex items-center mb-4 mr-[auto] sm:mx-[auto]">
                            <input type="radio" name="orderby" value="onsaleDate" onClick={Input}></input>
                            <label for="default-radio-1" class="ml-3 text-sm font-medium text-gray-900 dark:text-gray-300">On Sale Date</label>
                        </div>
                        <div class="flex items-center mb-4 mr-[auto] sm:mx-[auto]">
                            <input type="radio" name="orderby" value="issueNumber" onClick={Input}></input>
                            <label for="default-radio-1" class="ml-3 text-sm font-medium text-gray-900 dark:text-gray-300">Issue Number</label>
                        </div>
                ...
                <div className='w-full flex flex-col overflow-y-auto '>
                    ...
                    {status === STATUS.idle && records.map((elem) => {
                        return (<>
                            ...
                            <div className=' flex flex-col flex-shrink-0 w-[240px] h-[500px]  bg-[#011e1e] mt-[20px] mx-[auto]  lg:flex-row lg:w-[1000px] lg:h-[130px]' onClick={() => { navigate('./ComicView', { state: elem }) }}>
                                ...
                                <div className='flex flex-col  text-[white] h-[200px] w-[240px] lg:ml-[20px] lg:w-[700px] lg:h-[100px]'>
                                    <h1 className='font-bold text-[20px]'>{elem.title}</h1>
                                    <h1 className=' text-[grey] font-bold'>ISBN NUMBER -{elem.isbn}</h1>
                                    <p className='text-[grey] text-ellipsis overflow-hidden ... lg:truncate ...'>{elem.description}</p>
                                    <div className='flex flex-row text-[black] text-[15px] font-bold mt-[25px] lg:mt-[1px]'>

                                        <h1 className='w-[150px]  bg-[#ffffa3] rounded-[5px] lg:ml-[10px]'>ISSUE NO: {elem.issueNumber}</h1>
                                        <h2 className='w-[150px]  bg-[#ff9c9c] rounded-[5px] lg:ml-[10px]'>PAGE COUNT: {elem.pageCount}</h2>
                                    </div>
